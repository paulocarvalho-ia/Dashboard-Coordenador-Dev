...
def load_data():
    ...
    for col in df_bi.columns:
        ...
        elif 'ano e mês' in col or 'ano e mes' in col or 'ano_mês' in col or 'ano_mes' in col:
    ...
    # Merge SEGURO por codigo_cliente
...
# ============================================================
    ...
    if base_op and comp_op:
        ...
        if base_sem_vendas:
            st.warning(f"Indústrias sem vendas: {', '.join(base_sem_vendas)}.")
        else:
            ...
            if clientes_oportunidade:
                ...
                st.download_button("📥 Baixar Excel", data=output_op.getvalue(),
        ...
        st.info("Selecione indústrias em ambas as listas.")
...
# ============================================================
    ...
    if not df_softys.empty:
        ...
        pivot_mensal = df_softys_ano.pivot_table(index='Categoria', columns='Mês_Ano', 
        ...
        tabela = tabela[['Categoria'] + meses_ano + ['YTD']]
        
        st.markdown("**Positivação por Categoria**")
        ...
        st.download_button("📥 Baixar Excel", data=output_mensal.getvalue(),
        ...
        if pdf_mensal:
            st.download_button("📄 Baixar PDF", data=pdf_mensal,
                               file_name=f'softys_mensal_{datetime.now().strftime("%Y%m%d")}.pdf',
                               mime='application/pdf', use_container_width=True)
        
        st.markdown("**Batalha Naval Softys Falcon**")
        ...
        st.download_button("📥 Baixar Excel", data=output_bn.getvalue(),
        ...
        if pdf_bn:
            st.download_button("📄 Baixar PDF", data=pdf_bn,
...
# ============================================================
    ...
    if not df_perfumarias_ativas.empty:
        ...
        if not df_kenvue_mes.empty:
            ...
            st.metric("Atendidas Kenvue", f"{atendidos} ({pct_atendido:.1f}%)")
            ...
            with col_ken1:
                ...
                st.download_button("📥 Baixar Excel", data=output_cheg.getvalue(),
                ...
                if pdf_cheg:
                    st.download_button("📄 Baixar PDF", data=pdf_cheg,
            ...
            with col_ken2:
                ...
                st.download_button("📥 Baixar Excel", data=output_nao.getvalue(),
                ...
                if pdf_nao:
                    st.download_button("📄 Baixar PDF", data=pdf_nao,
            ...
            for vend in vendedores_perf:
                ...
                lista_ken.append({'Vendedor': vend, 'Perfumarias Ativas': total_vend, 
                                  'Atendidas Kenvue': atend_vend, '% Atendido': round(pct_vend, 1)})
            ...
            st.download_button("📥 Baixar Excel", data=output_kenv.getvalue(),
            ...
            if pdf_kenv:
                st.download_button("📄 Baixar PDF", data=pdf_kenv,
...
# ============================================================
    ...
    if not df_cenoura.empty:
        ...
        if mes_selecionado != "Todos":
            ...
            ano_atual = df_cenoura['Ano'].max()
            mes_atual_num = df_cenoura['Mês'].max()
        ...
        for i in range(1, 7):
            ...
            while m <= 0:
                m += 12
                a -= 1
            months_window.append(f"{a}-{m:02d}")
        
        current_month_str = f"{ano_atual}-{mes_atual_num:02d}"
        df_cenoura_window = df_cenoura[df_cenoura['Mês_Ano'].isin(months_window)]
        df_cenoura_mes = df_cenoura[df_cenoura['Mês_Ano'] == current_month_str]
        
        vendedores_cen = df_cenoura['nome_vendedor'].dropna().unique()
        lista_cen = []
        for vend in vendedores_cen:
            media_6m = df_cenoura_window[df_cenoura_window['nome_vendedor'] == vend].groupby('Mês_Ano')['codigo_cliente'].nunique().mean()
            clientes_mes = df_cenoura_mes[df_cenoura_mes['nome_vendedor'] == vend]['codigo_cliente'].nunique()
            pct = (clientes_mes / media_6m * 100) if media_6m > 0 else 0
            lista_cen.append({'Vendedor': vend, 'Média 6M': round(media_6m, 1), 
                              'Mês Atual': clientes_mes, '% Mês vs Média': round(pct, 1)})
...
# ============================================================
    ...
    st.subheader("📋 Batalha Naval — Cliente × Indústria")
    ...
    if not df_filtrado.empty:
        ...
        pivot_bn = df_bn.pivot_table(
            index=['codigo_cliente', 'nome_cliente', 'Municipio', 'Cliente_Coligacao', 'nome_vendedor'],
            columns='Nome_Fabricante', aggfunc='size', fill_value=0
        ).reset_index()
        ...
        pdf_bn = gerar_pdf_html(pivot_bn, "Batalha Naval")
        if pdf_bn:
            st.download_button("📄 Baixar PDF", data=pdf_bn,
...
# ============================================================
    ...
    if clientes_disponiveis:
        ...
        if cliente_selecionado:
            df_cliente = df_filtrado[df_filtrado['nome_cliente'] == cliente_selecionado]
            
            if not df_cliente.empty:
                col_f1, col_f2 = st.columns(2)
                with col_f1:
                    st.markdown("**Dados**")
                    st.write(f"Código: {df_cliente['codigo_cliente'].iloc[0]}")
                    st.write(f"Coligação: {df_cliente['Cliente_Coligacao'].iloc[0]}")
                    st.write(f"Município: {df_cliente['Municipio'].iloc[0]}")
                    st.write(f"Canal: {df_cliente['Canal'].iloc[0]}")
                    st.write(f"Segmento: {df_cliente['Segmento'].iloc[0]}")
                
                with col_f2:
                    st.markdown("**Performance**")
                    st.write(f"Pedidos: {len(df_cliente)}")
                    st.write(f"Indústrias: {df_cliente['Nome_Fabricante'].nunique()}")
                    st.write(f"Categorias: {df_cliente['Categoria'].nunique()}")
                
                st.markdown("**Histórico**")
                df_hist = df_cliente.groupby(['Mês_Ano', 'Nome_Fabricante']).size().reset_index(name='Pedidos')
